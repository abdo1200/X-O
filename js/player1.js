...
function back(){
	...
	num=0;
	inputs=["n1","n2","n3","n4","n5","n6","n7","n8","n9"];
}
...
let inputs=["n1","n2","n3","n4","n5","n6","n7","n8","n9"];
function Computer(){
	
	if(num % 2 == 1 && num<9 && v==""){
		for(let i=0;i<inputs.length;i++)
		{
			let checker=document.getElementById(inputs[i]).value
			if(checker=="O"){
				let remove =inputs.splice(i,1);
				console.log(remove);
			}if(checker=="X"){
				let remove2 =inputs.splice(i,1);
				console.log(remove2);	
			}
		}
		index = Math.floor(Math.random()*inputs.length);
		checker=document.getElementById(inputs[index]).value;
		if(checker=="O"|| checker=="X"){
			inputs.splice(index,1);
			index = Math.floor(Math.random()*inputs.length);
			document.getElementById(inputs[index]).value="X";
		}else{
			document.getElementById(inputs[index]).value="X";
		}
		document.getElementById("playOne").style.display = "block";
